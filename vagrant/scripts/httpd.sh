...
sed -e 's/^DocumentRoot "\/var\/www\/html"$/DocumentRoot "\/srv\/dav-prc"/' -i /etc/httpd/conf/httpd.conf
sed -e 's/^<Directory "\/var\/www\/html">$/<Directory "\/srv\/dav-prc">/' -i /etc/httpd/conf/httpd.conf

# Adjust server name in /etc/httpd/conf/httpd.conf
sed -e 's/^#ServerName www.example.com:80$/ServerName dav-prc-vagrant/' -i /etc/httpd/conf/httpd.conf
